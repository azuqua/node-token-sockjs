...
var TokenSocketServer = function(options){
	...
	var requestToken = function(req, res){
		async.waterfall([
			function(callback){
				if(self.debug)
					console.log("Sockjs: Sockjs token request received ", req.query);
			...
			if(error){
				if(self.debug) console.log("Sockjs: Error issuing token", error);
				res[type](500, { error: error.message || error });
			}else if(!auth){
				if(self.debug) console.log("Sockjs: Rejected token request");
				...
				utils.issueToken(self.redisClient, save, function(error, token){
					if(self.debug) console.log("Sockjs: Attaching token to data: ", token, save);
					if(error)
						res[type](500, { error: error.message || error });
					else
	...
	if(options.customMiddleware){
		if(self.debug) 
			console.log("Sockjs: Creating token route and attaching custom middleware");
		self.app.get(self.tokenRoute, options.customMiddleware, requestToken);
	}else{
		if(self.debug)
			console.log("Sockjs: Creating token route");
		self.app.get(self.tokenRoute, requestToken);
	}

	if(options.pubsubClient){
		if(self.debug) 
			console.log("Sockjs: Setting up publish-subscribe functionality");
		...
		self.pubsubClient.on("message", function(channel, message){
			if(channel === ALL_CHANNEL){
				if(self.debug)
					console.log("Sockjs: Broadcasting message to all sockets on all channels", message);
				...
				if(self.debug)
					console.log("Sockjs: Pub-sub message received: ", channel, message);
	...
	if(self.cleanup)
		self.enableCleanup(self.cleanup);

	self.socketServer.on("connection", function(socket){
		if(self.debug) console.log("Sockjs: Socket connection initiated");
		var sid = uuid.v4();
		socket.sid = sid;
		...
		socket.on("close", function(){
			if(self.debug)
				console.log("Sockjs: Socket connection closed", sid);
		...
		socket.on("data", function(message){
			...
			if(self.debug)
				console.log("Sockjs: Socket message received", message);
			...
			var sendMessage = function(error, resp){
				if(error)
					...
					message.resp = resp;
				if(self.debug)
					console.log("Sockjs: Writing message on socket", socket.sid, message);
				socket.write(JSON.stringify(message));
			};

			if(message.rpc === "auth" && message.token){
				if(self.debug)
					console.log("Sockjs: Socket auth request received", message.token);
				...
				utils.verifyToken(self.redisClient, message.token, function(error, data){
					if(error || !data){
						if(self.debug)
							console.log("Sockjs: Error verifying token ", message.token, error);
						sendMessage(error.message || error || "Invalid token");
						delete self._sockets[socket.sid];
						socket.end();
					}else{
						if(self.debug)
							console.log("Sockjs: Successfully verified token ", message.token, data);
						socket.auth = data;
				...
				if(self.debug)
					console.log("Sockjs: Invalid socket message. Ending connection", sid, message);
				...
				if(message.rpc && message.rpc === "_subscribe"){
				...
				}else if(message.rpc && message.rpc === "_rpc"){
					...
					if(action && typeof action === "function")
						action(socket.auth, message.req, sendMessage, socket);
					else
						sendMessage("Function not found");
				}else{
...
// expose a subset of the event emitter functions

TokenSocketServer.prototype.on = TokenSocketServer.prototype.addListener = function(event, callback){
	this._emitter.on(event, callback);
};

TokenSocketServer.prototype.removeListener = function(event, listener){
	this._emitter.removeListener(event, listener);
};

TokenSocketServer.prototype.removeAllListeners = function(event){
	this._emitter.removeAllListeners(event);
};

TokenSocketServer.prototype.rpc = function(socket, command, args, callback){
	var fid = uuid.v4();
...
TokenSocketServer.prototype.enableCleanup = function(interval){
	if(this.debug)
		console.log("Sockjs: Enabling cleanup process");
	...
	if(!(this.cleanupFn && typeof this.cleanupFn === "function")){
		this.cleanupFn = function(instance){
			if(instance.debug)
				console.log("Sockjs: Running cleanup process");
			...
			async.each(instance.sockets() || [], function(socket, callback){
				if(!socket.auth && socket.created && socket.created < diff){
					if(instance.debug)
						console.log("Sockjs: Cleaning up socket", socket.sid);
...
TokenSocketServer.prototype.disableCleanup = function(){
	if(this.debug)
		console.log("Sockjs: Disabling cleanup");
	this.cleanup = null;
	if(this.cleanupTimer)
		clearInterval(this.cleanupTimer);
};
...
TokenSocketServer.prototype.subscribe = function(socket, channel){
	if(this.pubsubClient){
		if(this.debug)
			console.log("Sockjs: Subscribing socket ", socket.sid, channel);
...
TokenSocketServer.prototype.unsubscribe = function(socket, channel){
	if(this.pubsubClient && socket && channel){
		if(this.debug)
			console.log("Sockjs: Unsubscribing socket ", socket.sid, channel);
...
TokenSocketServer.prototype.unsubscribeAll = function(channel){
	if(this.pubsubClient && channel && this._channels[channel]){
		var self = this;
		_.each(self._channels[channel], function(socket){
			self.unsubscribe(socket, channel);
		});
	}
};
...
TokenSocketServer.prototype.publish = function(channel, data){
	if(this.pubsubClient){
		if(this.debug)
			console.log("Sockjs: Publishing message", channel, data);
...
TokenSocketServer.prototype.broadcast = function(data){
	if(this.pubsubClient){
		var self = this;
		if(self.debug)
			console.log("Sockjs: Broadcasting message", data);
		if(typeof data === "object")
			data = JSON.stringify(data);
...
TokenSocketServer.prototype.shutdown = function(){
	...
	if(self.pubsubClient){
		...
		if(self.debug)
			console.log("Sockjs: Unsubscribed from all channels");
	}
};
