...
var TokenSocketServer = function(options){
	...
	if(options.customMiddleware){
		if(self.debug) console.log("Sockjs: Creating token route and attaching custom middleware");
		self.app.get(self.tokenRoute, options.customMiddleware, requestToken);
	}else{
		if(self.debug) console.log("Sockjs: Creating token route");
		self.app.get(self.tokenRoute, requestToken);
	}

	if(options.pubsubClient){
		if(self.debug) console.log("Sockjs: Setting up publish-subscribe functionality");
			...
			if(message.rpc === "auth" && message.token){
				...
				utils.verifyToken(self.redisClient, message.token, function(error, data){
					if(error || !data){
						...
						if(typeof self._onAuthentication === "function")
							self._onAuthentication(auth, socket);
					}
				});
			}else if(!socket.auth){
				if(self.debug)
...
TokenSocketServer.prototype.onAuthentication = function(callback){
	this._onAuthentication = callback;
};
